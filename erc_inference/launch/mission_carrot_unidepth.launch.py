"""mission_carrot.launch.py with UniDepthV2 as the free-space depth model.

Same carrot controller, same side-step and local replanning; the only change is
...
Every mission_carrot argument works here and means the same, e.g.

    ros2 launch erc_inference mission_carrot_unidepth.launch.py obstacle_sidestep:=true
...
def generate_launch_description():
    return LaunchDescription([
        LogInfo(msg='mission_carrot with depth_backend:=unidepth (UniDepthV2)'),
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(os.path.join(
                get_package_share_directory('erc_inference'), 'launch', 'mission_carrot.launch.py')),
