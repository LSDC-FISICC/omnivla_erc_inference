...
class CheckpointControllerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('proximity_threshold_m', 5.0)
    ...
    def _execute_callback(self, goal_handle):
        ...
        try:
            ...
            while checkpoint_index < len(checkpoints):
                ...
                self._publish_goal(checkpoint)
                self.get_logger().info(
                    f'Navigating to checkpoint sequence {checkpoint.sequence} '
                    f'({checkpoint.latitude:.8f}, {checkpoint.longitude:.8f}).'
                )
                if not self._wait_for_checkpoint(goal_handle, checkpoint):
                    goal_handle.canceled()
                    result.message = 'Mission canceled.'
                    return result
